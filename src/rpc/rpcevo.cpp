...
UniValue protx_list(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
                "protx_list_wallet (detailed wallet_only valid_only height)\n"
                "\nLists all ProTxs.\n"
                "\nArguments:\n"
                ...
                "\nExamples:\n"
                + HelpExampleCli("protx_list_wallet", "")
                + HelpExampleCli("protx_list_wallet", "true false false 200000")
        );
    }
...
UniValue protx_update_registrar(const JSONRPCRequest& request)
{
    ...
    const CKey& ownerKey = strOwnKey.empty() ? GetKeyFromWallet(pwalletMain, dmn->pdmnState->keyIDOwner)
                                             : ParsePrivKey(pwalletMain, strOwnKey, false);
...
{ //  category       name                              actor (function)
  //  -------------- --------------------------------- ------------------------
    ...
    { "evo",         "protx_revoke",                   &protx_revoke,           {}  },
    { "evo",         "protx_update_registrar",         &protx_update_registrar, {}  },
