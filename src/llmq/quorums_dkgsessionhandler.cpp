...
bool CDKGSessionHandler::InitNewQuorum(const CBlockIndex* pindexQuorum)
{
    ...
    auto mns = utils::GetAllQuorumMembers(params.type, pindexQuorum);
...
{
    LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - starting, curPhase=%d, nextPhase=%d\n", __func__, params.name, curPhase, nextPhase);
    ...
    while (true) {
        ...
        if (curPhase != QuorumPhase_None && p.first != curPhase) {
            LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - aborting due unexpected phase change\n", __func__, params.name);
            throw AbortPhaseException();
        }
        if (!runWhileWaiting()) {
            MilliSleep(100);
    ...
    LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - done, curPhase=%d, nextPhase=%d\n", __func__, params.name, curPhase, nextPhase);

}

void CDKGSessionHandler::WaitForNewQuorum(const uint256& oldQuorumHash)
{
    LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - starting\n", __func__, params.name);
    ...
    LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - done\n", __func__, params.name);
}
                                          ...
                                          const WhileWaitFunc& runWhileWaiting)
{
    ...
    LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - starting sleep for %d ms, curPhase=%d\n", __func__, params.name, sleepTime, curPhase);

    while (GetTimeMillis() < endTime) {
        if (stopRequested) {
            LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - aborting due to stop/shutdown requested\n", __func__, params.name);
            throw AbortPhaseException();
        }
        {
            LOCK(cs);
            ...
            if (phase != curPhase || quorumHash != expectedQuorumHash) {
                ...
                LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - aborting due unexpected phase/expectedQuorumHash change\n", __func__, params.name);
                throw AbortPhaseException();
            }
        }
        if (!runWhileWaiting()) {
    ...
    LogPrint(BCLog::DKG, "CDKGSessionManager::%s -- %s - done, curPhase=%d\n", __func__, params.name, curPhase);
}
...
void CDKGSessionHandler::HandleDKGRound()
{
    ...
    auto fComplainStart = [this]() {
        curSession->VerifyAndComplain(pendingContributions);
    };
    ...
    auto fJustifyStart = [this]() {
        curSession->VerifyAndJustify(pendingContributions);
    };
    ...
    auto fCommitStart = [this]() {
        curSession->VerifyAndCommit(pendingContributions);
    };
