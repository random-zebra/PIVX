...
bool CQuorumBlockProcessor::ProcessCommitment(int nHeight, const uint256& blockHash, const CFinalCommitment& qc, CValidationState& state, bool fJustCheck)
{
    ...
    const uint256& quorumHash = chainActive.Tip() != nullptr ? GetQuorumBlockHash((Consensus::LLMQType)qc.llmqType, nHeight)
                                                             : qc.quorumHash;
    ...
    const CBlockIndex* quorumIndex = mapBlockIndex.at(qc.quorumHash);
    auto members = utils::GetAllQuorumMembers(params.type, quorumIndex);
    if (!qc.Verify(members, false)) {
