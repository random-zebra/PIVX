...
static void addMnToLastPaid(const CMasternodeMan* manager,
                            std::vector<std::pair<int64_t, MasternodeRef>>& vecMasternodeLastPaid,
                            bool fFilterSigTime, const MasternodeRef& mn, int minProtocol, int nMnCount,
                            int nBlockHeight, const CBlockIndex* pBlockReading)
{
    // check protocol version
    if (mn->protocolVersion < minProtocol) return;

    // it's in the list (up to 8 entries ahead of current block to allow propagation) -- so let's skip it
    if (masternodePayments.IsScheduled(*mn, nBlockHeight)) return;

    // it's too new, wait for a cycle
    if (fFilterSigTime && mn->sigTime + (nMnCount * 2.6 * 60) > GetAdjustedTime()) return;

    // make sure it has as many confirmations as there are masternodes
    if (pcoinsTip->GetCoinDepthAtHeight(mn->vin.prevout, nBlockHeight) < nMnCount) return;

    vecMasternodeLastPaid.emplace_back(manager->SecondsSincePayment(mn, pBlockReading), mn);
}
    ...
    */
    ...
    int nMnCount = mnList.GetValidMNsCount();
    {
        ...
        for (const auto& it : mapMasternodes) {
            ...
            addMnToLastPaid(this, vecMasternodeLastPaid, fFilterSigTime, it.second, minProtocol, nMnCount, nBlockHeight, BlockReading);
        }
    }
    // Add deterministic masternodes to the vector
    mnList.ForEachMN(true, [&](const CDeterministicMNCPtr& dmn) {
        const MasternodeRef mn = MakeMasternodeRefForDMN(dmn);
        addMnToLastPaid(this, vecMasternodeLastPaid, fFilterSigTime, mn, minProtocol, nMnCount, nBlockHeight, BlockReading);
    });
...
static void updateWinner(MasternodeRef& winner, int64_t& score, const uint256& hash, const MasternodeRef& mn)
{
...
MasternodeRef CMasternodeMan::GetCurrentMasterNode(int nHeight, const uint256& hash) const
{
    ...
    for (const auto& it : mapMasternodes) {
        if (!it.second->IsEnabled()) continue;
        updateWinner(winner, score, hash, it.second);
    }
    ...
    if (deterministicMNManager->IsDIP3Enforced()) {
        ...
        mnList.ForEachMN(true, [&](const CDeterministicMNCPtr& dmn) {
            const MasternodeRef mn = MakeMasternodeRefForDMN(dmn);
            updateWinner(winner, score, hash, mn);
        });
    }
...
static void addMnToScores(std::vector<std::pair<int64_t, CTxIn>>& vecMasternodeScores,
                         const uint256& hash, const MasternodeRef& mn, int minProtocol)
{
    if (mn->protocolVersion < minProtocol) {
        LogPrint(BCLog::MASTERNODE,"Skipping Masternode with obsolete version %d\n", mn->protocolVersion);
        return; // Skip obsolete versions
    }
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
        if (GetAdjustedTime() - mn->sigTime < MN_WINNER_MINIMUM_AGE) {
            return; // Skip masternodes younger than (default) 1 hour
        }
    }
...
int CMasternodeMan::GetMasternodeRank(const CTxIn& vin, int64_t nBlockHeight) const
{
    ...
    std::vector<std::pair<int64_t, CTxIn> > vecMasternodeScores;
    {
        ...
        for (const auto& it : mapMasternodes) {
            if (!it.second->IsEnabled()) continue;
            addMnToScores(vecMasternodeScores, hash, it.second, minProtocol);
        }
    }
    ...
    if (deterministicMNManager->IsDIP3Enforced(nBlockHeight)) {
        ...
        mnList.ForEachMN(true, [&](const CDeterministicMNCPtr& dmn) {
            const MasternodeRef mn = MakeMasternodeRefForDMN(dmn);
            addMnToScores(vecMasternodeScores, hash, mn, minProtocol);
        });
    }
...
static void addMnToScores(std::vector<std::pair<int64_t, MasternodeRef>>& vecMasternodeScores,
                          const uint256& hash, const MasternodeRef& mn)
{
    ...
    if (!hash) return vecMasternodeScores;
    {
        ...
        for (const auto& it : mapMasternodes) {
            ...
            if (!mn->IsEnabled()) {
                ...
                addMnToScores(vecMasternodeScores, hash, mn);
            }
        }
    }
    // scan also dmns
