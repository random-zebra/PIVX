...
BOOST_AUTO_TEST_CASE(test_in_block_and_mempool_notes_double_spend)
{
    ...
    for (int i = 1; i <= 5; ++i) {
        CreateAndProcessBlock({}, scriptPubKey, true);
    }
    SyncWithValidationInterfaceQueue();
    ...
    const CBlock& block = CreateBlock({operation3.getFinalTx()}, scriptPubKey, true);
    CValidationState state;
    BOOST_CHECK(ProcessNewBlock(state, nullptr, std::make_shared<const CBlock>(block), nullptr));
    BOOST_CHECK(!state.IsValid());
    BOOST_CHECK(FormatStateMessage(state) == "bad-txns-sapling-requirements-not-met (code 16)");
    SyncWithValidationInterfaceQueue();
    {
        LOCK(cs_main);
        // Same tip as before, no block connection
        BOOST_CHECK(chainActive.Tip()->GetBlockHash() != block.GetHash());
        BOOST_CHECK_EQUAL(chainActive.Tip()->nHeight, 115);
    }
}
