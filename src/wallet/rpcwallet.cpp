...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    CWDestination dest = Standard::DecodeDestination(strAdd);
    // Make sure the destination is valid
    if (!Standard::IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }

    const CTxDestination* pTransDest = boost::get<CTxDestination>(&dest);
    ...
    if (spk_man && pTransDest) {
        ...
        // shield destination
        const libzcash::SaplingPaymentAddress pa = *boost::get<libzcash::SaplingPaymentAddress>(&dest);
