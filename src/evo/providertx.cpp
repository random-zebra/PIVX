...
static bool CheckService(const uint256& proTxHash, const ProTx& proTx, CValidationState& state)
{
    ...
    if (!Params().IsRegTestNet() && !proTx.addr.IsRoutable())
        return state.DoS(10, false, REJECT_INVALID, "bad-protx-addr");
...
static bool CheckInputsHashAndSig(const CTransaction &tx, const ProTx& proTx, const CKeyID &keyID, CValidationState& state)
{
    ...
    if (!CHashSigner::VerifyHash(::SerializeHash(proTx), keyID, proTx.vchSig, strError))
        return state.DoS(100, error("%s : %s", __func__, strError), REJECT_INVALID, "bad-protx-sig");
...
bool CheckProRegTx(const CTransaction& tx, CValidationState& state)
{
    ...
    if ((ptx.addr != CService() || ptx.nProtocolVersion != 0) && !CheckService(tx.GetHash(), ptx, state))
        return false;
