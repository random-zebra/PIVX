                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
    ...
    if (!ContextualCheckTransaction(_tx, state, params, nextBlockHeight, IsInitialBlockDownload())) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const auto& txIn : block.vtx) {
        ...
        if (Params().GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_V5_0) && tx.ContainsZerocoins()) {
            return state.DoS(100, error("%s : v5 upgrade enforced, zerocoin disabled", __func__));
        }
    }
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    for (const auto& tx : block.vtx) {
        ...
        // Check transaction contextually against consensus rules at block height
        if (!ContextualCheckTransaction(tx, state, chainparams, nHeight, IsInitialBlockDownload())) {
            return false;
        }
