 ...
 */
static bool ActivateBestChainStep(CValidationState& state, CBlockIndex* pindexMostWork, const CBlock* pblock, bool fAlreadyChecked, std::list<CTransaction>& txConflicted, std::vector<std::tuple<CTransaction,CBlockIndex*,int>>& txChanged, bool& fInvalidFound)
{
    AssertLockHeld(cs_main);
    if (pblock == NULL)
        fAlreadyChecked = false;
...
bool ActivateBestChain(CValidationState& state, const CBlock* pblock, bool fAlreadyChecked)
{
    ...
    do {
        ...
        while (true) {
            ...
            if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : nullptr, fAlreadyChecked, txConflicted, txChanged, fInvalidFound))
                return false;
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (hash != consensus.hashGenesisBlock && mapBlockIndex.find(block.hashPrevBlock) == mapBlockIndex.end()) {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    if (state.IsError())
                        break;
                } else if (hash != consensus.hashGenesisBlock && mapBlockIndex[hash]->nHeight % 1000 == 0) {
