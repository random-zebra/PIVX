...
BOOST_FIXTURE_TEST_CASE(dip3_protx, TestChain400Setup)
{
    ...
    {
        int randomIdx = 2 + InsecureRandRange(dmnHashes.size() - 2);
        const uint256& proTx = dmnHashes[randomIdx];
        auto tx = CreateProUpServTx(utxos, proTx, operatorKeys.at(proTx), 1, CScript(), coinbaseKey);
