...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        for (unsigned int inpIdx = 0; inpIdx < inputs.size(); inpIdx++) {
            ...
            BOOST_CHECK_MESSAGE(CheckTransaction(tx, false, false, state, false, false), strTest);
...
BOOST_AUTO_TEST_CASE(tx_invalid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        for (unsigned int inpIdx = 0; inpIdx < inputs.size(); inpIdx++) {
            ...
            CValidationState state;
            fValid = CheckTransaction(tx, false, false, state, false, false) && state.IsValid();
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests)
{
    ...
    stream >> tx;
    CValidationState state;
    BOOST_CHECK_MESSAGE(CheckTransaction(tx, false, false, state, false, false) && state.IsValid(), "Simple deserialized transaction should be valid.");

    // Check that duplicate txins fail
    tx.vin.push_back(tx.vin[0]);
    BOOST_CHECK_MESSAGE(!CheckTransaction(tx, false, false, state, false, false) || !state.IsValid(), "Transaction with duplicate txins should be invalid.");
}
