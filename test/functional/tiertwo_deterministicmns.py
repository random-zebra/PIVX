...
class DIP3Test(PivxTestFramework):
    ...
    def create_block(self, mn_payee_script, height, prevhash, sapling_root):
        coinbase = create_coinbase(height)
        coinbase.vout[0].nValue -= 3 * COIN
        coinbase.vout.append(CTxOut(int(3 * COIN), hex_str_to_bytes(mn_payee_script)))
        coinbase.rehash()
        return create_block(int(prevhash, 16), coinbase, hashFinalSaplingRoot=int(sapling_root, 16), nVersion=10)
    ...
    def run_test(self):
        ...
        sync_mempools([miner, controller])
        miner.generate(1)
        sync_blocks(self.nodes)
        ...
        prev_block = miner.getblock(miner.getbestblockhash(), True)
        block = self.create_block(mn_payee_script, prev_block["height"] + 1, prev_block["hash"], prev_block["finalsaplingroot"])
